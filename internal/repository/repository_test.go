...
func TestMain(m *testing.M) {
	...
	if err = pool.Retry(func() error {
		dbrm, err = gorm.Open(postgres.Open(postgresUrl), &gorm.Config{})
		if err != nil {
			log.Error("can't connect to the postgres: %w", err)
		}

		d, err := dbrm.DB()
		if err != nil {
			log.Error("can't get db from gorm instence: %w", err)
		}
...
func TestMigrate(t *testing.T) {
	...
	assert.Nil(t, err, "TestMigrate failed")
}
